...
const postsDirectory = path.join(path.resolve(), "src/content/posts");
const JSONFilePath = path.join(path.resolve(), "src/content/posts.json");
...
export const addNewCategories = (category: string) => {}


export const addNewPost = (post: Post, content: string): void => {
	const { posts, categories } = getJSON();

	const newAllPosts = {
		categories: categories,
		posts: Array.from(new Set([...posts, post]))
	}
